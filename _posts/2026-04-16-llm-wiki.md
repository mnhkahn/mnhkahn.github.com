...
在[Cyeam.com](https://www.cyeam.com/)任意网站点击页面头部的「求助架构师」或者输入cmd+K，即可调用 LLM Wiki 功能。
...
| 维度        | 快速模式（fast）                    | 思考模式（think）                                                                                          | 专家模式（expert）                                                                                                                    |
| ----------- | ----------------------------------- | ---------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| 数据获取    | Qdrant 向量搜索                     | MCP Wiki 工具链式查询                                                                                      | Qdrant + Wiki 双渠道                                                                                                                  |
| 搜索逻辑    | 单次 QdrantSearch，返回 Top2        | wiki_query → 提取 [[文章名]] → wiki_get_article                                                            | 先 Qdrant 向量搜索，再 Wiki 链式查询                                                                                                  |
| Thinking    | ❌ 无                                | ✅ 实时流式展示工具调用过程                                                                                 | ✅ 实时流式两阶段展示：1. 阶段一：向量搜索结果 2. 阶段二：Wiki 链式查询                                                                |
| Prompt 角色 | 熟悉 Golang/AI 的架构师             | 同快速模式                                                                                                 | 10 年经验资深系统架构师                                                                                                               |
| 字数限制    | ≤400 字                             | ≤400 字                                                                                                    | 不限                                                                                                                                  |
| 分析要求    | 基于资料总结                        | 同快速模式                                                                                                 | 多维度深度分析（技术选型、性能、可扩展性、安全） + 代码示例 + 方案对比                                                                |
| 计算过程    | QdrantSearch SiliconFlow 流式生成   | 纯文本追加                                                                                                 | 纯文本追加                                                                                                                            |
| 执行流程    | QdrantSearch → SiliconFlow 流式生成 | wiki_query(question) → 提取 [[文章名]] → wiki_get_article → 【实时 flush thinking】 → SiliconFlow 流式生成 | QdrantSearch → 【实时 flush 阶段一 thinking】 → wiki_query → wiki_get_article → 【实时 flush 阶段二 thinking】 → SiliconFlow 流式生成 |
